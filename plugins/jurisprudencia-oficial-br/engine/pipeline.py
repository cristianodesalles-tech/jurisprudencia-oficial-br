...
from .evidence import EvidenceGrader
from .overruling import OverrulingRegistry
...
class IngestionPipeline:
    def __init__(self, repository: Repository, object_store: ObjectStore, embedder: Embedder,
                 audit: HashChainAudit | None = None, grader: EvidenceGrader | None = None,
                 overruling: OverrulingRegistry | None = None):
        self.repository = repository
        self.object_store = object_store
        self.embedder = embedder
        self.audit = audit
        self.grader = grader or EvidenceGrader()
        self.overruling = overruling if overruling is not None else OverrulingRegistry()
    ...
    def ingest(self, document: JudicialDocument, raw_content: bytes, suffix: str = ".bin") -> IngestionResult:
        ...
        superado = self.overruling.annotate(document)
        decision = self.grader.grade(document, raw_content)
        document.status = decision.status
        document.metadata = {**document.metadata, "raw_object_key": raw_key, "evidence": decision.to_dict()}
        ...
        if self.audit:
            self.audit.append("document_ingested", {
                ...
                "evidence": decision.to_dict(), "overruling": superado,
            })
        return result
